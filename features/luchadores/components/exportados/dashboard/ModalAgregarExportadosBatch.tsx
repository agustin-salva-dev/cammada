...
export function ModalAgregarExportadosBatch({
  ...
  const luchadoresFiltrados = React.useMemo(() => {
    ...
    return luchadoresDisponibles.filter((l) => {
      const full =
        `${l.nombre} ${l.apodo} ${l.apellido} ${l.equipo?.nombre || ""}`.toLowerCase();
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-xl max-h-[85vh] flex flex-col gap-0 p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-border">
          ...
          <DialogDescription>
            Buscá y seleccioná múltiples luchadores para agregarlos todos juntos
            a la sección pública &quot;Talento Exportado&quot;.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="px-6 py-4 flex flex-col gap-3 border-b border-border bg-muted/20">
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground px-1">
            <span>
              {luchadoresFiltrados.length} luchador
              {luchadoresFiltrados.length !== 1 ? "es" : ""} disponible
              {luchadoresFiltrados.length !== 1 ? "s" : ""}
            </span>
